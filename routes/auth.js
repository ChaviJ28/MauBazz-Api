            ...
            if (err) {
                ...
                if (results.length > 0) {
                    if (bcrypt.compareSync(req.body.data.password, results[0].pwd)) {
                        if (results[0].is_active == 1 || results[0].login_count == 0) {
                            delete results[0].pwd;
                            results[0].access_type = "owner";
                            results[0].contact = JSON.parse(results[0].contact);
                            results[0].banner_url = JSON.parse(results[0].banner_url);
                            //update login_count
                            ...
                            connection.query(sql, async(err, sts) => {
                                if (err) {
                                    ...
                                    if (sts.affectedRows > 0) {
                                        ...
                                        res.json(
                                            await response.error(500, "Login Error, login_count fail")
                                        );
                                    }
                                }
                            });
